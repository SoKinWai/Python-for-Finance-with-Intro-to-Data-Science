...
class LoanPool(object):
    ...
    def Total_principal(self):
        logging.debug('Total principal is being calculated...')
        logging.debug('Sum up all the loans‘ principals')
    ...
    def Total_balance(self, period):
        logging.debug('Total balance is being calculated...')
        logging.debug('Sum up all the loans‘ balances')
    ...
    # WAR() function
    def WAR(self):
        logging.debug('The Weighted Average Rate of the loan pool is being calculated...')

        logging.debug('Create a list to store loan amounts and rates.')
        # Create a list to store loan amounts and rates.
        vals = [(loan.getRate(), loan.notional) for loan in self.loans]

        logging.debug('Using the sum of each face value times each rate, then divided by total principal.')
    ...
    # WAM() function
    def WAM(self):
        logging.debug('The Weighted Average Maturity (term) of the loan pool is being calculated...')

        logging.debug('Create a list to store loan amounts and terms.')
        # Create a list to store loan amounts and terms.
        vals = [(loan.notional, loan.term) for loan in self.loans]

        logging.debug('Using the sum of each face value times each term, then divided by total principal.')
        return reduce(lambda total, loan: total + loan[0] * loan[1] / float(self.Total_principal()), vals, 0)