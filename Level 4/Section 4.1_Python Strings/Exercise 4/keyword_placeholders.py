...
#Date: 04/23/2022
#4.1.4
#Format function with keyword placeholders‘ version
...
def main():
    ...
    try:
        ...
        if age_a > 0 and name_a != '' and residency_a != '':
            print('{name} is {age} years old and lives in {residence}.'.format(name=name_a,age=age_a,residence=residency_a))
        else:
    ...
    try:
        ...
        if age_b > 0 and name_b != '' and residency_b != '':
            print('{name} is {age:,.1f} years old and lives in {residence}.'.format(name=name_b,age=age_b,residence=residency_b))
        else:
            raise ValueError('Age has to be greater than 0. Name and residency can not be empty.')
    ...
    try:
        ...
        if age_c > 0 and name_c != '' and residency_c != '':
            print('{name} is {age:,.1f} years old and lives in {residence}.'.format(name=name_c,age=age_c,residence=residency_c.upper()))
        else:
            raise ValueError('Age has to be greater than 0. Name and residency can not be empty.')
    except ValueError as ex:
        print('ValueError exception:', ex, '\nPlease try again.')
