...
def main():
    ...
    try:
        ...
        if age_a > 0 and name_a != '' and residency_a != '':
            print(f'{name_a} is {age_a} years old and lives in {residency_a}.')
        else:
    ...
    try:
        ...
        if age_b > 0 and name_b != '' and residency_b != '':
            print(f'{name_b} is {age_b:,.1f} years old and lives in {residency_b}.')
        else:
            raise ValueError('Age has to be greater than 0. Name and residency can not be empty.')
    ...
    try:
        ...
        if age_c > 0 and name_c != '' and residency_c != '':
            print(f'{name_c} is {age_c:,.1f} years old and lives in {residency_c.upper()}.')
        else:
            raise ValueError('Age has to be greater than 0. Name and residency can not be empty.')
    ...
    #Which is cleanest?
    #I think f-Strings is the cleanest because I did not have to use a lot of codes.
