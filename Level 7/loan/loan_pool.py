...
class LoanPool(object):
    ...
    def getWaterfall(self,period):
        ...
        return [period,self.Total_principalDue(period),self.Total_interestDue(period),self.Total_monthlyPayment(period),self.checkDefaults(period),self.Total_balance(period)]




    # Create a method in LoanPool (called checkDefaults) that generates a list of uniform random integers, one for each Loan.
    # The parameter to this method should be a number.
    def checkDefaults(self, period):

        time_period=[10,59,60,120,180,210,360]

        default_pro=[0.0005, 0.001, 0.002, 0.004, 0.002, 0.001]


        #[period > a for a in time_period]  will return somthing like that [True, True, False.....]
        #True is 1 and False is 0. Then we can use the sume of the list as the index to find the probability
        index=sum([period > a for a in time_period])
        ...
        numbers = np.random.randint(0, 1 / default_pro[index], len(self.loans))

        return sum([loan.checkDefault(period,number) for loan, number in zip(self.loans,numbers)])
