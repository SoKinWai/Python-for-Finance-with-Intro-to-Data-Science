...
class Loan(object):
    ...
    def asset(self, iasset):
        if iasset > 0:
            self._asset = iasset  # Set the instance variable notional from input
        else:
            self._asset = '\nError: Please make sure the input of asset is greater than 0.'
    ...
    def calcBalance(cls,face, rate, term, period):
        ...
        if period >= 0 and period <= term:
            return face*((1+Loan.monthlyRate(rate))**period)-cls.calcMonthlyPmt(face, rate, term)*(((1+Loan.monthlyRate(rate))**period)-1)/(Loan.monthlyRate(rate))
        else:
            print("Error: Period has to be within the term of the loan and greater than or equal to 0.")
